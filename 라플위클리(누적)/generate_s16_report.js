...
let pending = [];
try { pending = JSON.parse(fs.readFileSync(path.join(DIR, 'weekly_cumulative_pending.json'), 'utf8').replace(/^﻿/, '')); } catch (e) { /* 없으면 무시 */ }

// 시즌1~6 — VR·참여도·트래픽소스까지 전부 유효한(결측 없는) 시즌 전체 대상.
// 시즌6은 아직 진행 중이라 발행 후 7일이 안 지난 마지막 편(집계대기)만 별도 제외(pending.json).
const SEASON_DEFS = [
  ...
  { full: '위클리 시즌6', id: 's6', label: '시즌6', short: 'S6', color: '#22D3EE' },
];
...
<header class="page-header">
  <p class="header-eyebrow">라플위클리 · 시즌1~6 상세 분석 (시즌6 진행 중, 결측 없는 편 전체)</p>
  <h1 class="header-title">시즌1~6 딥다이브 — 무엇이 성적을 갈랐나</h1>
  <p class="header-sub">VR·참여도·트래픽소스까지 전부 유효한 ${sAll.length}편으로 성과 드라이버를 역산${pending.length ? ` (${pending.map(p => `${p.season.replace('위클리 ', '')} ${p.num} ${p.title}`).join(', ')}은 발행 후 7일 미경과로 집계대기 제외)` : ''}</p>
...
<div class="section">
  <div class="card">
    ...
    <p class="card-sub">시즌1~6 전체 완전 데이터 기준 · 최저 ${worstSeason.label}(${worstSeason.avgV2.toFixed(3)}) ~ 최고 ${bestSeason.label}(${bestSeason.avgV2.toFixed(3)})</p>
...
<div class="section">
  <div class="card">
    ...
    <p class="card-sub">시즌1~6 ${sAll.length}편을 놓고 무엇이 점수를 갈랐는지 살펴본 결과</p>
    <ol class="key-list">
...
<div class="section">
  <div class="section-header">
    ...
    <p class="section-desc">LTV Score 내림차순 · 시즌1~6 전체 완전 데이터</p>
  </div>
  <div class="card" style="padding:0">
    <div class="tbl-scroll">
      <table class="ep-tbl">
...
fs.writeFileSync(path.join(DIR, '라플위클리_시즌16_상세분석.html'), html, 'utf8');
console.log('저장 완료: 라플위클리_시즌16_상세분석.html');
