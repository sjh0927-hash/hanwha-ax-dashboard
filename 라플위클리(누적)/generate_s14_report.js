...
// 시즌1~4 — VR·참여도·트래픽소스까지 전부 유효한(결측 없는) 완결 시즌만 대상.
// 시즌5는 3개 지표 결측, 시즌6은 진행 중(집계대기 포함)이라 이 딥다이브에서는 제외.
const SEASON_DEFS = [
...
const GRADE_CLASS = { S: 'gs', A: 'ga', B: 'gb', C: 'gc' };
...
function buildSeasonData(arr, other, label) {
  ...
  return {
    ...
    gradeCounts: {
      ...
      B: arr.filter(r => r.grade === 'B').length, C: arr.filter(r => r.grade === 'C').length,
    },
  };
}
...
function seasonPanel(id, data) {
  ...
  <div class="kpi-strip" style="grid-template-columns:repeat(3,1fr)">
    <div class="kpi-card purple">
      ...
      <span class="kpi-badge">${data.count}편 · S${data.gradeCounts.S} A${data.gradeCounts.A} B${data.gradeCounts.B} C${data.gradeCounts.C}</span>
    </div>
...
<header class="page-header">
  <p class="header-eyebrow">라플위클리 · 시즌1~4 상세 분석 (완결 시즌, 데이터 100% 확보)</p>
  <h1 class="header-title">시즌1~4 딥다이브 — 무엇이 성적을 갈랐나</h1>
  ...
  <div class="header-meta">
    ...
    <span class="meta-chip">등급분포 S${gradeTotal.S} A${gradeTotal.A} B${gradeTotal.B} C${gradeTotal.C}</span>
...
<div class="section">
  <div class="card">
    ...
    <p class="card-sub">시즌1~4 전체 완전 데이터 기준 · 최저 ${worstSeason.label}(${worstSeason.avgV2.toFixed(3)}) ~ 최고 ${bestSeason.label}(${bestSeason.avgV2.toFixed(3)})</p>
...
<div class="section">
  <div class="card">
    ...
    <p class="card-sub">시즌1~4 ${sAll.length}편을 놓고 무엇이 점수를 갈랐는지 살펴본 결과</p>
    <ol class="key-list">
...
<div class="section">
  <div class="section-header">
    ...
    <p class="section-desc">LTV Score 내림차순 · 시즌1~4 전체 완전 데이터</p>
  </div>
  <div class="card" style="padding:0">
    <div class="tbl-scroll">
      <table class="ep-tbl">
...
function seasonAvgChart(id, data){
  new Chart(document.getElementById(id),{type:'bar',
    ...
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>{const d=data[c.dataIndex];return 'LTV '+d.avgV2.toFixed(3)+' · S'+d.gradeCounts.S+' A'+d.gradeCounts.A+' B'+d.gradeCounts.B+' C'+d.gradeCounts.C;}}}},
      scales:{x:{grid:{display:false},ticks:TICK},y:{grid:{color:GRID},ticks:{...TICK,callback:v=>v.toFixed(1)},min:0,max:1}}}});
}
...
const SEASON_AVG=[${seasonAvgs.map(s => `{label:'${s.label}',avgV2:${s.avgV2.toFixed(3)},color:'${s.color}',gradeCounts:{S:${s.gradeCounts.S},A:${s.gradeCounts.A},B:${s.gradeCounts.B},C:${s.gradeCounts.C}}}`).join(',')}];
...
fs.writeFileSync(path.join(DIR, '라플위클리_시즌14_상세분석.html'), html, 'utf8');
console.log('저장 완료: 라플위클리_시즌14_상세분석.html');
