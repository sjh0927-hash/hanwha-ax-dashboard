...
18. **(2026-07-07) `$CC_MAP`(전환 소재별)도 16번과 같은 고정-컬럼-오프셋 문제로 신규 소재 데이터가 다른 소재 키에 덮어써짐** — 전환 캠페인에 신규 소재 딥다이브 8화 "장송의프리렌"이 투입되면서, 스크립트가 `$CC_MAP`의 고정 컬럼(`c=24` 등, `$convCreativeMonthRow` 기준)으로 '토이스토리' 데이터를 읽었는데 실제로는 그 컬럼이 신규 소재 쪽으로 밀려 있어서 **토이스토리 키에 장송의프리렌의 값이 그대로 들어감**(군체·스필버그는 우연히 컬럼이 안 밀려서 정상). 16번에서 클릭최대화(`$JUNE_LIST`→`$CLICK_CREATIVES`+`Find-CreativeBlocks`)는 고쳤지만 `$CC_MAP`/`$RC_MAP`은 아직 옛날 방식 그대로다 — **다음에 전환/후속조회 캠페인에 신규 소재가 생기면 똑같은 사고가 날 수 있으니, 그때는 `$CC_MAP`/`$RC_MAP`도 `Find-CreativeBlocks` 기반으로 교체할 것.** 이번엔 HTML만 수동으로 응급 수정(토이스토리 실제값 복구 + 장송의프리렌 별도 키 추가)하고 스크립트는 아직 안 고침.
